...
const Crash_Training = () => {
  ...
  const fetchCrash = async () => {
    setLoading(true);
    try {
      const res = await post(`/api/skill_test/all0`);
      console.log(res);
      if (res) {
        setCandate(res.data);
      }
  ...
  const handleSubmit = async () => {
    ...
    console.log(payload);
    try {
      const res = await post(`api/skill_test/update`, payload);
      console.log(res);
      if (res.success) {
        fetchCrash();
        setModals(false);
  ...
  return (
    <div className="lg:mt-10 mt-2">
      <div className="flex justify-between items-center ">
        <div className="">
          <h2 className="font-bold text-[24px] "> Candidates</h2>
        </div>
      </div>
      ...
      <Modal modals={modals} setModals={setModals}>
        ...
        <div className="flex justify-center">
          <button
            ...
            className="bg-[#1E3767] px-[40px] py-[12px] rounded-lg transition-transform active:scale-95 text-white"
          >
            Submit
          </button>
        </div>
