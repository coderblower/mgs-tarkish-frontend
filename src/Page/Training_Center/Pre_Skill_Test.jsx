...
const Pre_Skill_Test = () => {
  ...
  const fetchCandidate = async () => {
    setLoading(true);
    try {
      const res = await post(`/api/pre_skill_test/all?page=${currentPage}`);
      console.log(res);
      if (res) {
        setCandate(res.data);
        setPaginations({
  ...
  const handleSubmit = async () => {
    ...
    console.log(payload);
    try {
      const res = await post(`api/skill_test/create`, payload);
      console.log(res);
      if (res.success) {
        setModals(false);
        toast.success(res.message);
        qualified == 1
          ? navigate("/training/skill_test")
          : navigate("/training/crash_training");
      }
    } catch (err) {
      toast.error("failed!");
      console.log(err);
  ...
  return (
    <div className="lg:mt-10 mt-2">
      ...
      <Modal modals={modals} setModals={setModals}>
        <h1 className="text-center text-xl font-bold pb-2">Pre Skilled Test</h1>
