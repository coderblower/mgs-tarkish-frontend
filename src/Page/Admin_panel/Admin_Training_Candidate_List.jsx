...
const Admin_Training_Candidate_List = () => {
  ...
  const fetchCandidate = async (search, page) => {
    ...
    try {
      ...
      const res = await post(`/api/user/search_training_candidate?page=${search ? 1 : page}`, {
        pg: "a",
        phone: search,
        agent: agent,
  ...
  const preloadCandidates = async () => {
    for (let i = 1; i <= 4; i++) {
      ...
      try {
        ...
        const res = await post(`/api/user/search_training_candidate?page=${nextPage}`, {
          pg: "a",
          phone: search,
          country: parseInt(countryResult) || "",
        });
  ...
  const handleCSVData = async () => {
    setLoading(true);
    try {
      const res = await post(`api/user/search_training_candidate`, {
        pg: "a",
        phone: search,
        agent: agent,
        country: parseInt(countryResult) || "",
...
export default Admin_Training_Candidate_List;
